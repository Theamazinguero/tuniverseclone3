...
// keep these for possible debug, no UI wired right now
...
async function shareToCommunity() {
    ...
    const regionBox = $("artistsByCountryList");  // <-- share Artists by Region
    const displayName = $("displayNameInput") ? $("displayNameInput").value.trim() : "";
    const appToken = getAppToken();

    if (!msgBox || !regionBox) return;
    ...
    const summaryText = regionBox.textContent || "";
